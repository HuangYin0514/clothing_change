...
def plot_parameter_lossWeight():
    ...
    # 图参数
    max_values = max([max(m, r) for m, r in zip(map_values, rank1_values)]) * 1.2
    min_values = min([min(m, r) for m, r in zip(map_values, rank1_values)]) * 0.8
    ...
    # 绘制图像
    # axis_1.plot(x_values, map_values, label="mAP", color="#71D4EB", marker="o", markersize=MARKERSIZE, zorder=2)
    axis_1.plot(x_values, map_values, label="mAP", marker="o", markersize=MARKERSIZE, zorder=2)
    ...
    # 保存
    # plt.show()
    output_file = os.path.join(OUTPUT_DIR, "parameter_lossWeight_1.png")
